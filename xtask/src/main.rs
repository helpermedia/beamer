...
/// Directly references the plugin-specific extension class to ensure it's linked.
...
// Forward declare the plugin-specific extension class
@interface {extension_class} : NSObject
@end

int main(int argc, char *argv[]) {{
    @autoreleasepool {{
        // Reference the extension class to ensure ObjC runtime registration
        (void)[{extension_class} class];

        // App extensions communicate via XPC. The system handles loading
        // our NSExtensionPrincipalClass which implements the AUAudioUnitFactory
        // protocol to create AU instances.
        [[NSRunLoop mainRunLoop] run];
    }}
    return 0;
}}
"#, plugin_name = plugin_name, extension_class = extension_class)
}
...
fn create_appex_info_plist(config: &AppexPlistConfig) -> String {
    ...
    // Generate plugin-specific extension class name
