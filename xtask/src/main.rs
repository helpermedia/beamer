...
/// The view controller class inherits from AUViewController (required for AUv3 XPC).
fn generate_au_extension_source(plugin_name: &str) -> String {
    let pascal_name = to_pascal_case(plugin_name);
    let wrapper_class = format!("Beamer{}AuWrapper", pascal_name);
    let viewcontroller_class = format!("Beamer{}AuViewController", pascal_name);
    let factory_func = format!("Beamer{}AuViewControllerFactory", pascal_name);
...
// AUv3 requires the principal class to inherit from AUViewController,
// even for headless (no UI) plugins. This is required for XPC communication.
@interface {viewcontroller_class} : AUViewController <AUAudioUnitFactory>
@end

@implementation {viewcontroller_class}
...
void beamer_au_appex_force_link(void) {{
    (void)[{viewcontroller_class} class];
}}
...
"#, plugin_name = plugin_name,
    ...
    viewcontroller_class = viewcontroller_class,
    factory_func = factory_func)
}

/// Generate appex_main.m for the plugin.
/// Directly references the plugin-specific view controller class to ensure it's linked.
fn generate_appex_main_source(plugin_name: &str) -> String {
    let pascal_name = to_pascal_case(plugin_name);
    let viewcontroller_class = format!("Beamer{}AuViewController", pascal_name);

    format!(r#"// Auto-generated appex main for {plugin_name}
// DO NOT EDIT - Generated by xtask

#import <Foundation/Foundation.h>

// Forward declare the plugin-specific view controller class
@interface {viewcontroller_class} : NSObject
@end

int main(int argc, char *argv[]) {{
    @autoreleasepool {{
        // Reference the view controller class to ensure ObjC runtime registration
        (void)[{viewcontroller_class} class];

        // App extensions communicate via XPC. The system handles loading
        // our NSExtensionPrincipalClass (AUViewController subclass) which
        // implements AUAudioUnitFactory protocol to create AU instances.
        [[NSRunLoop mainRunLoop] run];
    }}
    return 0;
}}
"#, plugin_name = plugin_name, viewcontroller_class = viewcontroller_class)
}
...
fn get_au_rustflags(
    ...
    let flags = vec![
        ...
        format!("-C link-arg=-Wl,-exported_symbol,_OBJC_CLASS_$_{}", viewcontroller_class),
        format!("-C link-arg=-Wl,-exported_symbol,_OBJC_METACLASS_$_{}", wrapper_class),
        format!("-C link-arg=-Wl,-exported_symbol,_OBJC_METACLASS_$_{}", viewcontroller_class),
...
"#, framework_name = framework_name, bundle_id = framework_bundle_id, version = version_string);
    ...
    // Generate plugin-specific appex_main.m with unique force_link function
    let appex_main_source = generate_appex_main_source(package);
    ...
    let appex_main_path = appex_gen_dir.join("appex_main.m");
    fs::write(&appex_main_path, appex_main_source)
        .map_err(|e| format!("Failed to write appex_main.m: {}", e))?;
...
fn create_appex_info_plist(config: &AppexPlistConfig) -> String {
    ...
    // Generate plugin-specific view controller class name (must inherit from AUViewController)
...
<dict>
    ...
    <dict>
        ...
        <string>{viewcontroller_class}</string>
